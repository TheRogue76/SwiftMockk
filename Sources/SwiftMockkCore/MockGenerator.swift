...
public struct MockGenerator {
    ...
    public func generateMocks(for protocols: [ProtocolInfo], moduleName: String?) -> String {
        ...
        for proto in protocols {
            allImports.formUnion(proto.imports)
        }

        // Remove standard imports (we always add these)
        allImports.remove("Foundation")
        allImports.remove("SwiftMockk")

        // Remove module name if present (we add it as @testable)
        if let moduleName = moduleName {
            allImports.remove(moduleName)
        }
        ...
        // Add collected imports from source files
        for importName in allImports.sorted() {
            output += "import \(importName)\n"
        }
